...
dependency "api_skolerom_certificate" {
  config_path = "../api_skolerom_certificate"
...
# generate "api-ingress-providers" {
#   path      = "providers.tf"
...
inputs = {
  ...
  namespace          = "${local.common_vars.environment}-${local.common_vars.application}-backend-api"
  lb_name            = local.common_vars.lb_name
  record             = dependency.infrastructure.outputs.alb_dns_name
  service_name       = "${local.common_vars.environment}-${local.common_vars.application}-api-nginx-svc"
  ingress_group_name = "${local.common_vars.application}-${local.common_vars.environment}-ingress-group"
  certificate_arn    = dependency.api_skolerom_certificate.outputs.certificate_arn
  ...
  healthcheck_path   = "/api/classes"
  url                = local.common_vars.api_domain_name
}